...
typedef struct  {
    ...
    /* server.start()                                   开启服务端
    /* server.stop()                                    关闭服务端
    /* server.wait_for_start()                          等待直至服务端完全关闭（没显示，空函数）
    /* server.restart()                                 重启服务端
    /* server.stop_exit()                               关闭服务端以及 MCDR，也就是退出整个程序
    /* server.exit()                                    关闭 MCDR
    /* server.is_server_running()                       返回一个 bool 代表服务端
...
Server *NewServer(char *name, char *id, int memory, char *version, char *side, char *comment) {
    Server server = {name, id, memory, version, side, comment};
    return &server;
}